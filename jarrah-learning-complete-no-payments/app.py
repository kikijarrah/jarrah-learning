...
CREATE TABLE IF NOT EXISTS bookings(
 ...
 paid INTEGER NOT NULL DEFAULT 0, price_cents INTEGER NOT NULL DEFAULT 0,
 admin_fee_cents INTEGER NOT NULL DEFAULT 0, stripe_session_id TEXT, zoom_join_url TEXT,
...
def signup_student():
    if request.method == "POST":
        ...
        try:
            cur = con.execute(
                "INSERT INTO users(role,name,email,password_hash,created_at) VALUES(?,?,?,?,?)",
                ("student",f["name"],f["email"].lower(),generate_password_hash(f["password"]),datetime.utcnow().isoformat()),
            )
            uid = cur.lastrowid
            con.execute(
...
def signup_tutor():
    if request.method == "POST":
        ...
        try:
            cur = con.execute(
                "INSERT INTO users(role,name,email,password_hash,created_at) VALUES(?,?,?,?,?)",
                ("tutor",f["name"],f["email"].lower(),generate_password_hash(f["password"]),datetime.utcnow().isoformat()),
            )
            uid = cur.lastrowid
            photo = None
            ...
            con.execute(
                ...
                (uid,f.get("age"),f.get("subjects"),f.get("highest_level"),f.get("format"),f.get("tutoring_type"),
...
def student_dashboard():
    ...
    con = db()
    s = con.execute(
        "SELECT u.name,u.email,p.* FROM users u JOIN student_profiles p ON p.user_id=u.id WHERE u.id=?",
        (uid,),
    ).fetchone()
    tutors = con.execute(
        "SELECT u.id,u.name,p.* FROM users u JOIN tutor_profiles p ON p.user_id=u.id WHERE p.approved=1"
    ).fetchall()
    matches = sorted([(score_match(s,t),t) for t in tutors], key=lambda x:x[0], reverse=True)
    bookings = con.execute(
        "SELECT b.*,u.name tutor_name FROM bookings b JOIN users u ON u.id=b.tutor_id WHERE b.student_id=? ORDER BY b.start_at DESC",
        (uid,),
    ).fetchall()
    assessments = con.execute(
        "SELECT * FROM assessments WHERE student_id=? ORDER BY assessment_date",
        (uid,),
    ).fetchall()
    con.close()
...
def tutor_detail(tutor_id):
    con = db()
    tutor = con.execute(
        "SELECT u.id,u.name,p.* FROM users u JOIN tutor_profiles p ON p.user_id=u.id WHERE u.id=? AND p.approved=1",
        (tutor_id,),
    ).fetchone()
    slots = con.execute(
        "SELECT * FROM availability WHERE tutor_id=? AND booked=0 AND start_at>? ORDER BY start_at",
        (tutor_id,datetime.now().isoformat()),
    ).fetchall()
    con.close()
    if not tutor:
        abort(404)
...
def book(slot_id):
    con = db()
    slot = con.execute(
        "SELECT a.*,u.name tutor_name,p.tutoring_type FROM availability a "
        "JOIN users u ON u.id=a.tutor_id JOIN tutor_profiles p ON p.user_id=u.id "
        "WHERE a.id=? AND a.booked=0",
        (slot_id,),
    ).fetchone()
    if not slot:
        con.close()
    ...
    if request.method == "POST":
        session_type = request.form.get("session_type","volunteer")
        cur = con.execute(
            "INSERT INTO bookings(student_id,tutor_id,availability_id,start_at,end_at,topic,strengths,weaknesses,status,paid,price_cents,admin_fee_cents,created_at) "
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (session["user_id"],slot["tutor_id"],slot_id,slot["start_at"],slot["end_at"],
             request.form.get("topic"),request.form.get("strengths"),request.form.get("weaknesses"),
             "confirmed",0,0,0,datetime.utcnow().isoformat()),
        )
        bid = cur.lastrowid
        ...
        con.execute("UPDATE availability SET booked=1 WHERE id=?", (slot_id,))
        con.commit()
        ...
        try:
            ...
            if z:
                con.execute(
                    "UPDATE bookings SET zoom_join_url=?,zoom_start_url=? WHERE id=?",
                    (z["join_url"],z["start_url"],bid),
                )
                con.commit()
        ...
        if session_type == "paid":
            flash("Session booked. Any tutoring fee is handled separately by e-transfer; Jarrah Learning does not process payments.","success")
        else:
...
def cancel_booking(booking_id):
    ...
    if not b or (role=="student" and b["student_id"]!=uid) or (role=="tutor" and b["tutor_id"]!=uid):
        ...
        abort(403)
    con.execute(
        "UPDATE bookings SET status='cancelled',cancellation_fee_cents=0 WHERE id=?",
        (booking_id,),
    )
    con.execute("UPDATE availability SET booked=0 WHERE id=?", (b["availability_id"],))
    con.commit()
    con.close()
...
def tutor_dashboard():
    ...
    con = db()
    p = con.execute(
        "SELECT u.*,p.* FROM users u JOIN tutor_profiles p ON p.user_id=u.id WHERE u.id=?",
        (uid,),
    ).fetchone()
    slots = con.execute(
        "SELECT * FROM availability WHERE tutor_id=? ORDER BY start_at DESC LIMIT 30",
        (uid,),
    ).fetchall()
    bookings = con.execute(
        "SELECT b.*,u.name student_name FROM bookings b JOIN users u ON u.id=b.student_id WHERE b.tutor_id=? ORDER BY b.start_at DESC",
        (uid,),
    ).fetchall()
    con.close()
...
def tutor_availability():
    ...
    con.execute(
        ...
        (session["user_id"],start.isoformat(timespec="minutes"),end.isoformat(timespec="minutes")),
    )
    con.commit()
    con.close()
...
def tutor_recap(booking_id):
    con = db()
    b = con.execute(
        "SELECT * FROM bookings WHERE id=? AND tutor_id=?",
    ...
    con.execute(
        ...
        (booking_id,session["user_id"],request.form["summary"],request.form.get("homework"),datetime.utcnow().isoformat()),
    )
    for up in request.files.getlist("files"):
        if up and up.filename:
            original = secure_filename(up.filename)
...
def admin_dashboard():
    ...
    pending = con.execute(
        "SELECT u.id,u.name,u.email,p.* FROM users u JOIN tutor_profiles p ON p.user_id=u.id WHERE p.approved=0"
    ).fetchall()
    bookings = con.execute(
        "SELECT b.*,s.name student_name,t.name tutor_name FROM bookings b "
        "JOIN users s ON s.id=b.student_id JOIN users t ON t.id=b.tutor_id "
...
def approve_tutor(tutor_id):
    con = db()
    con.execute("UPDATE tutor_profiles SET approved=1 WHERE user_id=?", (tutor_id,))
    con.commit()
    con.close()
...
def get_file(booking_id,file_id):
    con = db()
    f = con.execute(
        "SELECT bf.*,b.student_id,b.tutor_id FROM booking_files bf "
        "JOIN bookings b ON b.id=bf.booking_id WHERE bf.id=? AND bf.booking_id=?",
        (file_id,booking_id),
    ).fetchone()
    con.close()
    if not f:
        abort(404)
    if session["role"]!="admin" and session["user_id"] not in {f["student_id"],f["tutor_id"]}:
        abort(403)
    return send_from_directory(
        UPLOAD_DIR,f["stored_name"],as_attachment=True,download_name=f["filename"]
    )
